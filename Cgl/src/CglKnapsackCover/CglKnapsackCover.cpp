	...
	seqLiftAndUncomplementAndAdd(nCols, xstar, complement, rowIndex,
      ...
      if (expensiveCuts_ || krow.getNumElements() <= longRow) {
        if (findJohnAndEllisCover(rowIndex, krow, b,
              xstar, fracCover, atOnes, remainder)
          == 1) {
...
//-------------------------------------------------------------------
int CglKnapsackCover::liftAndUncomplementAndAdd(
  double /*rowub*/,
  CoinPackedVector &krow,
  double &b,
  int *complement,
...
//-------------------------------------------------------------------
int CglKnapsackCover::deriveAKnapsack(
  ...
  const double *element)
{
  ...
  if (krow.getNumElements() < 3 || nBinUnsat == 0 || maxKrowElement - minKrowElement < 1.0e-3 * maxKrowElement) {
    return 0;
  }
  ...
  if (b < 0) {
    ...
    const double fakeLb = colupper[index] + 1.0;
    ; // yes, colupper.
#ifdef CGL_DEBUG
...
// -----------------------------------------
int CglKnapsackCover::findLPMostViolatedMinCover(
  int nCols,
  int
#ifdef PRINT_DEBUG
    row
  ...
  CoinPackedVector &remainder)
{
  ...
  for (i = 0; i < krow.getNumElements(); i++) {
    if (fabs(krow.getElements()[i]) > epsilon_) {
      ratio[krow.getIndices()[i]] = (1.0 - xstar[krow.getIndices()[i]]) / (krow.getElements()[i]);
    } else {
...
// -----------------------------------------
int CglKnapsackCover::findExactMostViolatedMinCover(
  int nCols,
  int
#ifdef PRINT_DEBUG
    row
  ...
  CoinPackedVector &remainder)
{
  ...
  if (objConst - exactOptVal < 1) {
    ...
    // by testing and potentially tossing smallest
    // elements
    double oneLessCoverElementSum = coverElementSum - cover.getElements()[cover.getNumElements() - 1];
...
//-------------------------------------------------------------------
int CglKnapsackCover::findPseudoJohnAndEllisCover(
  int
#ifdef PRINT_DEBUG
    row
#endif
...
//-------------------------------------------------------------------
int CglKnapsackCover::findJohnAndEllisCover(
  int
#ifdef PRINT_DEBUG
    row
#endif
...
#ifdef CGL_DEBUG
      // Sanity check
      int size = (fracCover.getNumElements() + remainder.getNumElements() + atOne.getNumElements());
      int krowsize = krow.getNumElements();
      assert(size == krowsize);
#endif
...
//-------------------------------------------------------------------
int CglKnapsackCover::findGreedyCover(
  int
#ifdef PRINT_DEBUG
    row
#endif
...

{
  ...
  for (i = 0; i < krow.getNumElements(); i++) {
    ...
    if (xstar[krow.getIndices()[i]] >= epsilon_ && xstar[krow.getIndices()[i]] <= onetol_ && !gotCover) {
  ...
  if ((greedyXstarSum <= (cover.getNumElements() - 1) + epsilon2_) || (!gotCover) || (cover.getNumElements() < 2)) {
    return -1;
  }
...
#endif
void CglKnapsackCover::liftUpDownAndUncomplementAndAdd(
  int nCols,
  double *xstar,
  int *complement,
  int /*row*/,
...
//
//-------------------------------------------------------------------
void CglKnapsackCover::seqLiftAndUncomplementAndAdd(
  int nCols,
  double *xstar,
  int *complement,
  ...
  OsiCuts &cs)
{
  ...
  if (remainder.getNumElements() > 0) {
    ...
    for (j = 0; j < remainder.getNumElements(); j++) {
      ...
      ratio[remainder.getIndices()[j]] = (cutRhs - psi_j) / remainder.getElements()[j];
      CoinDecrSolutionOrdered dso(ratio);
      a.sort(dso);
      alpha.sort(dso);
    }
...
//
//-------------------------------------------------------------------
int CglKnapsackCover::liftCoverCut(
  double &b,
  int nRowElem,
  CoinPackedVector &cover,
  CoinPackedVector &remainder,
...
int CglKnapsackCover::gubifyCut(CoinPackedVector &cut)
{
  int goodCut = 0;
#ifdef GUBCOVER
  if (numberCliques_) {
...
//-------------------------------------------------------------------
int CglKnapsackCover::exactSolveKnapsack(
  int n,
  double c,
  double const *pp,
  double const *ww,
...
void CglKnapsackCover::refreshSolver(OsiSolverInterface *solver)
{
#ifdef GUBCOVER
  deleteCliques();
