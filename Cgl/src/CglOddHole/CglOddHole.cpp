...
#define MAXELS 50000
  ...
  double_double_int_triple *sortit = new double_double_int_triple[nCols];
  memset(mark, 0, nSmall2 * sizeof(int));
      ...
      if (good) {
        ...
        if (good) {
          ...
          if (violation < minimumViolation_) {
            ...
            if (static_cast< double >(ii) * minimumViolationPer_ > violation || ii > maximumEntries_) {
#ifdef CGL_DEBUG
              printf("why no cut\n");
#endif
              if (packed) {
...
void CglOddHole::refreshSolver(OsiSolverInterface *)
{
}
