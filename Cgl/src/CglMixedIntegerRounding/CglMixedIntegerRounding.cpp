...
// Delete
//-------------------------------------------------------------------
void CglMixedIntegerRounding::gutsOfDelete()
{
  if (vubs_ != 0) {
    delete[] vubs_;
    vubs_ = 0;
...
// Copy
//-------------------------------------------------------------------
void CglMixedIntegerRounding::gutsOfCopy(const CglMixedIntegerRounding &rhs)
{
  ...
  mixIntRoundPreprocess(const OsiSolverInterface &si)
{
  ...
  for (iRow = 0; iRow < numRows_; ++iRow) {
    ...
    const RowType rowType = determineRowType(si, rowLengths[iRow], colInds + rowStarts[iRow],
  ...
  for (iRow = 0; iRow < numRows_; ++iRow) {
    ...
    else if ((rowType == ROW_VARUB) || (rowType == ROW_VARLB) || (rowType == ROW_VAREQ)) {
...
// Generate MIR cuts
//-------------------------------------------------------------------
void CglMixedIntegerRounding::generateMirCuts(
  ...
  OsiCuts &cs) const
{
  ...
  // free memory
  delete[] listColsSelected;
  listColsSelected = 0;
  delete[] listRowsAggregated;
  listRowsAggregated = 0;
  delete[] xlpExtra;
  xlpExtra = 0;
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding::copyRowSelected(
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding::selectRowToAggregate(
  const OsiSolverInterface &si,
  ...
  int &colSelected) const
{
  ...
  for (int j = 0; j < numColsAggregated; ++j) {
    ...
    if ((!si.isContinuous(indCol)) || (fabs(coefCol) < EPSILON_))
      continue;

    // Compute current lower bound
    CglMixIntRoundVLB VLB = vlbs_[indCol];
    double LB = (VLB.getVar() != UNDEFINED_) ? VLB.getVal() * xlp[VLB.getVar()] : colLowerBound[indCol];

    // Compute current upper bound
    CglMixIntRoundVUB VUB = vubs_[indCol];
    double UB = (VUB.getVar() != UNDEFINED_) ? VUB.getVal() * xlp[VUB.getVar()] : colUpperBound[indCol];
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding::aggregateRow(
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding::boundSubstitution(
  const OsiSolverInterface &si,
  const CoinPackedVector &rowAggregated,
  const double *xlp,
  ...
  CoinPackedVector &contVariablesInS) const
{
  ...
  for (j = 0; j < numColsAggregated; ++j) {
    ...
    if ((indCol < numCols_) && (colLowerBound[indCol] == colUpperBound[indCol])) {
    ...
    if (indCol < numCols_) { // variable is model variable
      ...
      const double LB = (VLB.getVar() != UNDEFINED_) ? VLB.getVal() * xlp[VLB.getVar()] : colLowerBound[indCol];

      // Compute upper bound for variable indCol
      const CglMixIntRoundVUB VUB = vubs_[indCol];
      const double UB = (VUB.getVar() != UNDEFINED_) ? VUB.getVal() * xlp[VUB.getVar()] : colUpperBound[indCol];

      // if both bounds are infinite, then we cannot form a mixed knapsack
      if ((LB == -1.0 * si.getInfinity()) && (UB == si.getInfinity())) {
#if CGL_DEBUG
      ...
      if (isLowerSubst(si.getInfinity(), rowAggregatedElements[j],
        ...
        if (VLB.getVar() != UNDEFINED_) {
          ...
          if (mixedKnapsack.isExistingIndex(indVLB)) {
            ...
            mixedKnapsack.setElement(index, mixedKnapsack[indVLB] + coefCol * VLB.getVal());
          } else
        ...
        if (VUB.getVar() != UNDEFINED_) {
          ...
          if (mixedKnapsack.isExistingIndex(indVUB)) {
            ...
            mixedKnapsack.setElement(index, mixedKnapsack[indVUB] + coefCol * VUB.getVal());
          } else
...
#if CGL_DEBUG
  std::cout << "# of continuous var in mixedKnapsack = " << numCont << std::endl;
#endif
  if (numCont == 0)
    return generated;
...
#if CGL_DEBUG
  std::cout << "# of integer var in mixedKnapsack = " << numInt << std::endl;
#endif
  if (numInt == 0)
    return generated;
  ...
  for (j = 0; j < numInt; ++j) {
    ...
    if (fabs(knapsackElements[j]) < EPSILON_)
      continue;
    // if the lower bound is not zero, then we stop
    if (fabs(colLowerBound[knapsackIndices[j]]) > EPSILON_)
      return generated;
  }
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding::cMirSeparation(
  const OsiSolverInterface &si,
  const CoinPackedMatrix &matrixByRow,
  const CoinPackedVector &rowAggregated,
  ...
  OsiRowCut &cMirCut) const
{
  ...
  for (j = 0; j < numInt; ++j) {
    ...
    if (colUpperBound[indCol] != si.getInfinity()) {
      if (xlp[indCol] >= colUpperBound[indCol] / 2.0) {
        ...
        if ((xlp[indCol] <= EPSILON_) || (xlp[indCol] >= colUpperBound[indCol] - EPSILON_))
          continue;
  ...
  if (complTSize > 0) {
    ...
    for (int j = 0; j < complTSize; ++j) {
      ...
      double violation = 0.0;
      double sCoef = 0.0;
      double localNumeratorBeta = numeratorBeta - mixedKnapsack[indCol] * colUpperBound[indCol];
      ...
      if (isLowerSubst(si.getInfinity(), rowAggregated[indCol],
        ...
        if (VLB.getVar() != UNDEFINED_) {
          ...
          if (bestCut.isExistingIndex(indVLB)) {
            ...
            bestCut.setElement(index, bestCut[indVLB] - sCoefBestCut * coefCol * VLB.getVal());
          } else
        ...
        if (VUB.getVar() != UNDEFINED_) {
          ...
          if (bestCut.isExistingIndex(indVUB)) {
            ...
            bestCut.setElement(index, bestCut[indVUB] + sCoefBestCut * coefCol * VUB.getVal());
          } else
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding::cMirInequality(
  const int numInt,
  const double delta,
...
// Printing statistics
//-------------------------------------------------------------------
void CglMixedIntegerRounding::printStats(
  std::ofstream &fout,
  const bool hasCut,
  const OsiSolverInterface &si,
  const CoinPackedVector &rowAggregated,
  ...
  const double *colLowerBound) const
{
  ...
  for (int j = 0; j < numColsAggregated; ++j) {
    ...
    if ((indCol < numCols_) && (!si.isContinuous(indCol))) {
      ...
      // print
      fout << "I " << xlp[indCol] << " " << colLowerBound[indCol] << " " << colUpperBound[indCol] << std::endl;
    ...
    if (indCol < numCols_) { // variable is model variable
      ...
      // print
      fout << "C " << xlp[indCol] << " " << colLowerBound[indCol] << " " << colUpperBound[indCol] << " ";

      // variable lower bound?
      CglMixIntRoundVLB VLB = vlbs_[indCol];
      if (VLB.getVar() != UNDEFINED_) {
        fout << VLB.getVal() << " " << xlp[VLB.getVar()] << " " << colLowerBound[VLB.getVar()] << " " << colUpperBound[VLB.getVar()] << " ";
      } else {
        fout << "-1 -1 -1 -1 ";
      }
      ...
      CglMixIntRoundVUB VUB = vubs_[indCol];
      if (VUB.getVar() != UNDEFINED_) {
        fout << VUB.getVal() << " " << xlp[VUB.getVar()] << " " << colLowerBound[VUB.getVar()] << " " << colUpperBound[VUB.getVar()] << " ";
      } else {
        fout << "-1 -1 -1 -1 ";
      }
      ...
      // in this case the LB = 0 and the UB = infinity
      // print
      fout << "C " << xlpExtra[indCol - numCols_] << " " << 0.0 << " " << si.getInfinity() << " ";
    }
...
void CglMixedIntegerRounding::refreshSolver(OsiSolverInterface *)
{
  doneInitPre_ = false;
}
