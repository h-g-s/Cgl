...
void second_(float *t) { *t = CoinCpuTime(); }
#endif
...
)
{
  ...
  if (inp_ilp == NULL)
    alloc_error(const_cast< char * >("inp_ilp"));
...
void Cgl012Cut::get_parity_ilp()
{
  ...
  for (j = 0; j < inp_ilp->mc; j++) {
    ...
    if (xstarj > static_cast< double >(ubj - ZERO)) {
      ...
      if (mod2(ubj) == ODD) {
        ...
        if (parity_col_removed == EVEN)
          parity_col_removed = ODD;
        else
          parity_col_removed = EVEN;
      } else {
      ...
      if (loss_upper > loss_lower)
        p_ilp->min_loss_by_weak[j] = loss_lower;
      else
        p_ilp->min_loss_by_weak[j] = loss_upper;
    }
  }
  ...
  for (i = 0; i < inp_ilp->mr; i++) {
    ...
    if (slacki > MAX_SLACK - EPS)
      p_ilp->row_to_delete[i] = TRUE;
    else
      p_ilp->row_to_delete[i] = FALSE;
  ...
  for (i = 0; i < p_ilp->mr; i++)
    for (h = i + 1; h < p_ilp->mr; h++)
      if ((p_ilp->mrhs[i] == p_ilp->mrhs[h]) && (p_ilp->mtcnt[i] == p_ilp->mtcnt[h]) && (!p_ilp->row_to_delete[i]) && (!p_ilp->row_to_delete[h])) {
        begi = p_ilp->mtbeg[i];
        begh = p_ilp->mtbeg[h];
        equalih = TRUE;
...
separation_graph *Cgl012Cut::initialize_sep_graph()
{
  ...
  if (s_graph->nodes == NULL)
    alloc_error(const_cast< char * >("s_graph->nodes"));
  for (j = 0; j < nnodes; j++)
    s_graph->nodes[j] = nodes[j];
  free(nodes);
  s_graph->ind = reinterpret_cast< int * >(malloc(maxnodes * sizeof(int)));
  if (s_graph->ind == NULL)
    alloc_error(const_cast< char * >("s_graph->ind"));
  for (j = 0; j < maxnodes; j++)
    s_graph->ind[j] = ind[j];
  free(ind);
  maxedges = (nnodes * (nnodes - 1)) / 2;
  s_graph->even_adj_list = reinterpret_cast< edge ** >(malloc(maxedges * sizeof(edge *)));
  if (s_graph->even_adj_list == NULL)
    alloc_error(const_cast< char * >("s_graph->even_adj_list"));
  s_graph->odd_adj_list = reinterpret_cast< edge ** >(malloc(maxedges * sizeof(edge *)));
  if (s_graph->odd_adj_list == NULL)
    alloc_error(const_cast< char * >("s_graph->odd_adj_list"));
...
)
{
  ...
  if (parity == EVEN)
    old_edge = s_graph->even_adj_list[indjk];
  else
    old_edge = s_graph->odd_adj_list[indjk];
  if (old_edge == NULL) {
    /* edge is not in the graph */
    new_edge = reinterpret_cast< edge * >(calloc(1, sizeof(edge)));
    if (new_edge == NULL)
      alloc_error(const_cast< char * >("new_edge"));
    new_edge->endpoint1 = indj;
    new_edge->endpoint2 = indk;
    new_edge->weight = weight;
    new_edge->parity = parity;
    new_edge->constr = i;
    new_edge->weak = i_weak;
    (s_graph->nedges)++;
    if (parity == EVEN)
      s_graph->even_adj_list[indjk] = new_edge;
    else
      s_graph->odd_adj_list[indjk] = new_edge;
  } else {
...
auxiliary_graph *define_aux_graph(separation_graph *s_graph /* input separation graph */)
{
  ...
  if (a_graph == NULL)
    alloc_error(const_cast< char * >("a_graph"));
...
#endif
  if (a_graph->nodes == NULL)
    alloc_error(const_cast< char * >("a_graph->nodes"));
#ifndef CGL_NEW_SHORT
...
#endif
  if (a_graph->arcs == NULL)
    alloc_error(const_cast< char * >("a_graph->arcs"));
  ...
  for (j = 0; j < s_graph->nnodes; j++) {
    ...
    for (k = 0; k < s_graph->nnodes; k++)
      if (k != j) {
        ...
        if (s_graph->even_adj_list[indjk] != NULL)
          totoutj++;
        if (s_graph->odd_adj_list[indjk] != NULL)
          totoutj++;
      }
    auxj1 = AG_TWIN1(j);
    auxj2 = AG_TWIN2(j);
...
#ifndef CGL_NEW_SHORT
          a_graph->arcs[narcs].len = a_graph->arcs[narcs + totoutj].len = (int)(s_edge->weight * ISCALE);
          a_graph->arcs[narcs].head = &(a_graph->nodes[auxk1]);
          a_graph->arcs[narcs + totoutj].head = &(a_graph->nodes[auxk2]);
#else
          a_graph->arcs[narcs].length = a_graph->arcs[narcs + totoutj].length = static_cast< int >(s_edge->weight * ISCALE);
...
short int simple_cycle(cycle *s_cyc /* cycle to be checked */)
{
  ...
  if (cnt == NULL)
    alloc_error(const_cast< char * >("cnt"));
...
short int same_cycle(cycle *s_cyc1, cycle *s_cyc2 /* cycles to be compared */)
{
  ...
  if (s_cyc1->length != s_cyc2->length)
    return (FALSE);
...
cycle_list *initialize_cycle_list(int max_cyc /* maximum number of cycles in the list */)
{
  ...
  if (s_cycle_list == NULL)
    alloc_error(const_cast< char * >("s_cycle_list"));
  s_cycle_list->cnum = 0;
  s_cycle_list->list = reinterpret_cast< cycle ** >(calloc(max_cyc, sizeof(cycle *)));
  if (s_cycle_list->list == NULL)
    alloc_error(const_cast< char * >("s_cycle_list->list"));
  return (s_cycle_list);
}
...
)
{
  ...
  path_time += tf - ti;
#endif
  forw_arb = reinterpret_cast< short_path_node * >(calloc(a_graph->nnodes, sizeof(short_path_node)));
  if (forw_arb == NULL)
    alloc_error(const_cast< char * >("forw_arb"));
  for (k = 0; k < a_graph->nnodes; k++) {
#ifndef CGL_NEW_SHORT
    if (a_graph->nodes[k].parent != NULL) {
  ...
  backw_arb = reinterpret_cast< short_path_node * >(calloc(a_graph->nnodes, sizeof(short_path_node)));
  if (backw_arb == NULL)
    alloc_error(const_cast< char * >("backw_arb"));
  for (k = 0; k < a_graph->nnodes; k++) {
#ifndef CGL_NEW_SHORT
    if (a_graph->nodes[k].parent != NULL) {
  ...
  path_time += tf - ti;
#endif
  backw_arb = (short_path_node *)calloc(a_graph->nnodes, sizeof(short_path_node));
  if (backw_arb == NULL)
    alloc_error("backw_arb");
  for (k = 0; k < a_graph->nnodes; k++) {
    ...
    if (k != j) {
      for (t = 1; t <= 2; t++) {
        if (t == 1)
          ...
          kt = AG_TWIN2(k);
        weight = (static_cast< double >(forw_arb[kt].dist + backw_arb[kt].dist)) / (static_cast< double >(ISCALE));
...
cut_list *initialize_cut_list(int max_cut /* maximum number of cuts in the list */)
{
  ...
  if (cuts == NULL)
    alloc_error(const_cast< char * >("cuts"));
  cuts->cnum = 0;
...
{
  if (v_cut->constr_list != NULL)
    free(v_cut->constr_list);
  if (v_cut->in_constr_list != NULL)
    free(v_cut->in_constr_list);
  if (v_cut->cind != NULL)
    free(v_cut->cind);
  if (v_cut->cval != NULL)
    free(v_cut->cval);
...
)
{
  ...
  if (vars_to_weak == NULL)
    alloc_error(const_cast< char * >("vars_to_weak"));
...
)
{
  ...
  if (v_cut == NULL)
    alloc_error(const_cast< char * >("v_cut"));
  v_cut->crhs = crhs;
  cnzcnt = 0;
  for (j = 0; j < inp_ilp->mc; j++)
    if (ccoef[j] != 0)
  ...
  if (v_cut->cind == NULL)
    alloc_error(const_cast< char * >("v_cut->cind"));
  v_cut->cval = reinterpret_cast< int * >(calloc(cnzcnt, sizeof(int)));
  if (v_cut->cval == NULL)
    alloc_error(const_cast< char * >("v_cut->cval"));
  cnzcnt = 0;
  v_cut->violation = 0.0;
  for (j = 0; j < inp_ilp->mc; j++)
    if (ccoef[j] != 0) {
...
)
{
  ...
  iter++;
  if (v_cut->violation > violation + EPS || v_cut->violation < violation - EPS) {
...
)
{
  ...
  if (only_viol && viol < MIN_VIOLATION)
    return (-INF);
  norm = 0;
  for (j = 0; j < p_ilp->mc; j++) {
    if (ccoef[j] != 0)
      norm += ccoef[j] * ccoef[j];
  }
  if (viol > 0.0)
    return (viol / sqrt(static_cast< double >(norm)));
  else
    return (viol * sqrt(static_cast< double >(norm)));
}
...
short int same_cut(cut *cut1, cut *cut2 /* cuts to be compared */)
{
  ...
  if (cut1->cnzcnt != cut2->cnzcnt)
    return (FALSE);
  if (cut1->crhs != cut2->crhs)
    return (FALSE);
  if (cut1->csense != cut2->csense)
    return (FALSE);
  for (j = 0; j < cut1->cnzcnt; j++) {
    if (cut1->cind[j] != cut2->cind[j])
      return (FALSE);
    if (cut1->cval[j] != cut2->cval[j])
      return (FALSE);
  }
  return (TRUE);
}
...
)
{
  ...
  if (type_even_weak == NULL)
    alloc_error(const_cast< char * >("type_even_weak"));
  switch_even_weak = reinterpret_cast< short int * >(calloc(p_ilp->mc, sizeof(short int)));
  if (switch_even_weak == NULL)
    alloc_error(const_cast< char * >("switch_even_weak"));
  type_odd_weak = reinterpret_cast< short int * >(calloc(p_ilp->mc, sizeof(short int)));
  if (type_odd_weak == NULL)
    alloc_error(const_cast< char * >("type_odd_weak"));
  switch_odd_weak = reinterpret_cast< short int * >(calloc(p_ilp->mc, sizeof(short int)));
  if (switch_odd_weak == NULL)
    alloc_error(const_cast< char * >("switch_odd_weak"));
  ...
  for (ofsl = 0; ofsl < n_to_weak; ofsl++) {
    ...
    if (p_ilp->possible_weak[l] == NONE) {
      free(type_even_weak);
      free(type_odd_weak);
      free(switch_even_weak);
      free(switch_odd_weak);
#ifdef TIME
      second_(&tff);
      bw_time += tff - tii;
#endif
    ...
    if ((only_viol) && ((*best_even_slack) > MAX_SLACK - EPS) && ((*best_odd_slack) > MAX_SLACK - EPS)) {
      free(type_even_weak);
      free(type_odd_weak);
      free(switch_even_weak);
      free(switch_odd_weak);
  ...
  if ((!only_odd) && (((*best_even_slack) <= MAX_SLACK - EPS) || ((!only_viol) && (*best_even_slack) <= INF - EPS))) {
    ok_even = TRUE;
    ...
    for (ofsl = n_to_weak - 1; ofsl >= 0; ofsl--) {
      ...
      if (flag_even == EVEN) {
        ...
        if (switch_even_weak[cntweak])
          flag_even = ODD;
      } else {
        (*info_even_weak)->type[cntweak] = type_odd_weak[cntweak];
        if (switch_odd_weak[cntweak])
          flag_even = EVEN;
      }
    }
  } else
  ...
  if (((*best_odd_slack) <= MAX_SLACK - EPS) || ((!only_viol) && (*best_odd_slack) <= INF - EPS)) {
    ok_odd = TRUE;
    ...
    if (!p_ilp->row_to_delete[i]) {
      ...
      if (p_ilp->mtcnt[i] == 1) {
        ...
        if (weight < MAX_SLACK - EPS) {
          ...
          sep_graph = update_weight_sep_graph(j, special, weight, parity, i, i_weak, sep_graph);
        }
        ...
        if (weight < MAX_SLACK - EPS) {
          ...
          parity = p_ilp->mrhs[i];
          i_weak = alloc_info_weak(0);
          sep_graph = update_weight_sep_graph(j, k, weight, parity, i, i_weak, sep_graph);
        }
      } else {
          ...
          for (ofsk = ofsj + 1; ofsk < p_ilp->mtcnt[i]; ofsk++) {
            ...
            if (ok_weak == BOTH || ok_weak == EVEN) {
              if (best_even_slack < MAX_SLACK - EPS) {
                weight = best_even_slack;
                parity = EVEN;
                sep_graph = update_weight_sep_graph(j, k, weight, parity, i, info_even_weak, sep_graph);
              }
            }
            if (ok_weak == BOTH || ok_weak == ODD) {
              if (best_odd_slack < MAX_SLACK - EPS) {
                weight = best_odd_slack;
                parity = ODD;
                sep_graph = update_weight_sep_graph(j, k, weight, parity, i, info_odd_weak, sep_graph);
              }
            }
          EXITJK:;
          }
  ...
  for (j = 0; j < p_ilp->mc; j++) {
    if (!p_ilp->col_to_delete[j]) {
      ...
      if (weight < MAX_SLACK - EPS) {
        ...
        i_weak = alloc_info_weak(0);
        sep_graph = update_weight_sep_graph(j, special, weight, parity, NONE, i_weak, sep_graph);
      }
...
void initialize_cur_cut()
{
  ...
  if (cur_cut == NULL)
    alloc_error(const_cast< char * >("cur_cut"));
  cur_cut->coef = reinterpret_cast< int * >(calloc(n, sizeof(int)));
  if (cur_cut->coef == NULL)
    alloc_error(const_cast< char * >("cur_cut->coef"));
  cur_cut->non_weak_coef = reinterpret_cast< int * >(calloc(n, sizeof(int)));
  if (cur_cut->non_weak_coef == NULL)
    alloc_error(const_cast< char * >("cur_cut->non_weak_coef"));
  cur_cut->in_constr_list = reinterpret_cast< short int * >(calloc(m, sizeof(short int)));
  if (cur_cut->in_constr_list == NULL)
    alloc_error(const_cast< char * >("cur_cut->in_constr_list"));
  clear_cur_cut();
}
...
void initialize_hash_table()
{
  ...
  if (hash_tab == NULL)
    alloc_error(const_cast< char * >("hash_tab"));
...
void hash_insert()
{
  ...
  if (hash_el == NULL)
    alloc_error(const_cast< char * >("hash_el"));
  hash_el->n_of_el = m;
  hash_el->last_vis = it;
  hash_el->next = NULL;
  hash_el->flag_vect = reinterpret_cast< short int * >(calloc(m, sizeof(short int)));
  if (hash_el->flag_vect == NULL)
    alloc_error(const_cast< char * >("hash_el->flag_vect"));
  for (i = 0; i < m; i++)
...
void increase_prohib_period()
{
  if (prohib_period * 1.1 > prohib_period + 1)
    if (prohib_period * 1.1 < m - 2)
      prohib_period = static_cast< int >(prohib_period * 1.1);
    else
      prohib_period = m - 2;
  else if (prohib_period + 1 < m - 2)
    prohib_period += 1;
  else
    prohib_period = m - 2;
...
void decrease_prohib_period()
{
  if (prohib_period * 0.9 < prohib_period - 1)
    if (prohib_period * 0.9 > IN_PROHIB_PERIOD)
      prohib_period = static_cast< int >(prohib_period * 0.9);
    else
      prohib_period = IN_PROHIB_PERIOD;
  else if (prohib_period - 1 > IN_PROHIB_PERIOD)
    prohib_period -= 1;
  else
    prohib_period = IN_PROHIB_PERIOD;
  last_prohib_period_mod = it;
...
short int allowed(int i /* constraint to be checked */)
{
  if (last_moved[i] < it - prohib_period) {
    if (cur_cut->in_constr_list[i] == IN) {
      if (cur_cut->n_of_constr > 1)
        return (TRUE);
      else
        return (FALSE);
    } else {
      if (cur_cut->n_of_constr < m - 1)
        return (TRUE);
      else
        return (FALSE);
    }
...
{
  if (cur_cut->in_constr_list[i] == OUT)
    return (FALSE);
  else
    return (TRUE);
}
...
#define FAST_SCORE_EVAL 1
  ...
  if (itype == ADD)
    slack_sum += p_ilp->slack[i] / static_cast< double >(gcdi);
  else
    slack_sum -= p_ilp->slack[i] / static_cast< double >(gcdi);
...
*/
  ...
  for (ofsj = 0, ij = begi; ofsj < inp_ilp->mtcnt[i]; ofsj++, ij++)
    if (cur_cut->non_weak_coef[inp_ilp->mtind[ij]] != 0)
      support_inter++;
  ...
  if (new_coef == NULL)
    alloc_error(const_cast< char * >("new_coef"));

  if ((itype == ADD && inp_ilp->msense[i] != 'G') || (itype == DEL && inp_ilp->msense[i] == 'G'))
    flag_gt = 1;
  else
    flag_gt = -1;
  if (flag_gt == 1) {
  ...
  for (ofsj = 0, ij = begi; ofsj < inp_ilp->mtcnt[i]; ofsj++, ij++) {
    j = inp_ilp->mtind[ij];
    if (cur_cut->coef[j] > 0)
      one_norm -= cur_cut->coef[j];
    else
      one_norm += cur_cut->coef[j];
    if (new_coef[ofsj] >= 2)
      one_norm += new_coef[ofsj] / 2;
    else
      one_norm -= new_coef[ofsj] / 2;
...
*/
  ...
  if (ccoef == NULL)
    alloc_error(const_cast< char * >("ccoef"));
  for (j = 0; j < n; j++)
    ccoef[j] = cur_cut->non_weak_coef[j];
  for (ofsj = 0; ofsj < inp_ilp->mtcnt[i]; ofsj++) {
    ij = begi + ofsj;
    j = inp_ilp->mtind[ij];
  ...
  if (ok)
    score = tabu_score(ccoef, crhs, viol, static_cast< double >(one_norm));
  else {
...
)
{
  ...
  cur_cut->ok = best_cut(cur_cut->coef, &cur_cut->rhs, &cur_cut->violation, TRUE, FALSE);
  cur_cut->one_norm = 0;
  for (j = 0; j < n; j++) {
    if (cur_cut->coef[j] > 0)
      cur_cut->one_norm += cur_cut->coef[j];
    else
      cur_cut->one_norm -= cur_cut->coef[j];
  }
}
...
cut *Cgl012Cut::get_current_cut()
{
  ...
  if (cut_ptr == NULL)
    alloc_error(const_cast< char * >("cut_ptr"));
  ...
  if (cut_ptr->cind == NULL)
    alloc_error(const_cast< char * >("cut_ptr->cind"));
  cut_ptr->cval = reinterpret_cast< int * >(calloc(nz, sizeof(int)));
  if (cut_ptr->cval == NULL)
    alloc_error(const_cast< char * >("cut_ptr->cval"));
  nz = 0; /*viol = 0.0;*/
  for (j = 0; j < n; j++) {
    if (cur_cut->coef[j] != 0) {
      cut_ptr->cind[nz] = j;
  ...
  if (cut_ptr->constr_list == NULL)
    alloc_error(const_cast< char * >("cut_ptr->constr_list"));
  cut_ptr->in_constr_list = reinterpret_cast< short int * >(calloc(inp_ilp->mr, sizeof(short int)));
  if (cut_ptr->in_constr_list == NULL)
    alloc_error(const_cast< char * >("cut_ptr->in_constr_list"));
  for (i = 0; i < m; i++) {
    if (cur_cut->in_constr_list[i] == IN) {
...
void Cgl012Cut::add_tight_constraint()
{
  ...
  if (tight == NULL)
    alloc_error(const_cast< char * >("tight"));
  for (i = 0; i < m; i++) {
...
void Cgl012Cut::initialize()
{
  ...
  if (last_moved == NULL)
    alloc_error(const_cast< char * >("last_moved"));
  for (i = 0; i < m; i++) {
...
short int same_pool_cut(pool_cut *p_cut1, pool_cut *p_cut2)
{
  ...
  if (p_cut1->n_of_constr != p_cut2->n_of_constr)
    return (FALSE);
  if (p_cut1->code != p_cut2->code)
    return (FALSE);
  /* assumes constr_list is sorted for increasing/decreasing constraint index */
  for (c = 1; c < p_cut1->n_of_constr; c++)
    if (p_cut1->constr_list[c] != p_cut2->constr_list[c])
      return (FALSE);
  return (TRUE);
}
...
void free_pool_cut(pool_cut *p_cut)
{
  if (p_cut == NULL)
    return;
  if (p_cut->constr_list != NULL)
    free(p_cut->constr_list);
  free(p_cut);
}
...
void initialize_pool()
{
  ...
  if (pool == NULL)
    alloc_error(const_cast< char * >("pool"));
  pool->cnum = 0;
  pool->list = (pool_cut **)calloc(MAX_CUT_POOL, sizeof(pool_cut *));
  if (pool->list == NULL)
    alloc_error(const_cast< char * >("pool->list"));
  pool->ncod = (int *)calloc(MAX_CUT_COD, sizeof(int));
  if (pool->ncod == NULL)
    alloc_error(const_cast< char * >("pool->ncod"));
}
...
*/
  ...
  if (flag_v == NULL)
    alloc_error(const_cast< char * >("flag_v"));
  for (c = 0; c < pool->cnum; c++) {
    if (pool->list[c]->code != cod)
      continue;
    p_cut = pool->list[c];
    equal = TRUE;
    for (i = 0; i < inp_ilp->mr; i++)
...
void add_cuts_to_pool(cut_list *out_cuts)
{
  ...
  if (pool == NULL)
    initialize_pool();
  if (pool->cnum >= MAX_CUT_POOL * CLEAN_THRESH)
    clean_pool();
...
)
{
  ...
  if (interest_var == NULL)
    alloc_error(const_cast< char * >("interest_var"));
  n_interest_var = 0;
  for (j = 0; j < p_ilp->mc; j++) {
  ...
  if (best_var_cut == NULL)
    alloc_error(const_cast< char * >("best_var_cut"));
  for (k = 0; k < n_interest_var; k++) {
    best_var_cut[k] = (select_cut *)calloc(1, sizeof(select_cut));
    if (best_var_cut[k] == NULL)
      alloc_error(const_cast< char * >("best_var_cut[k]"));
  ...
  for (c = 0; c < pool->cnum; c++) {
    ...
    if (after_sep && p_cut->it_found != sep_iter)
      continue;
  ...
  if (maxscore > max_score_ever)
    max_score_ever = maxscore;
  ...
  if (added == NULL)
    alloc_error(const_cast< char * >("added"));
  for (c = 0; c < pool->cnum; c++)
    added[c] = FALSE;

  for (k = 0; k < n_interest_var; k++) {
    j = interest_var[k];
    if (!added[best_var_cut[k]->pool_index] && best_var_cut[k]->score >= MIN_CUT_SCORE) {
...
void Cgl012Cut::initialize_log_var()
{
  ...
  if (!vlog) {
    if (p_ilp->mc) {
      ...
      if (vlog == NULL)
        alloc_error(const_cast< char * >("vlog"));
      for (j = 0; j < p_ilp->mc; j++) {
        vlog[j] = reinterpret_cast< log_var * >(calloc(1, sizeof(log_var)));
        if (vlog[j] == NULL)
          alloc_error(const_cast< char * >("vlog[j]"));
        vlog[j]->n_it_zero = 0;
      }
    }
  } else {
