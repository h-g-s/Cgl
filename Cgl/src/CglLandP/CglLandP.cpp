...
CglLandP::CglLandP(const CglLandP::Parameters &params,
  ...
  , numcols_(-1)
  , originalColLower_(NULL)
  , originalColUpper_(NULL)
  , canLift_(false)
  , extraCuts_()
{
...
struct cutsCos {
  ...
  int j;
  double angle;
  cutsCos(int i_, int j_, double angle_)
    : i(i_)
    , j(j_)
    , angle(angle_)
  {
...
void CglLandP::scanExtraCuts(OsiCuts &cs, const double *colsol) const
{
  int numAdded = 0;
...
void CglLandP::generateCuts(const OsiSolverInterface &si, OsiCuts &cs,
  const CglTreeInfo info)
{
  ...
  for (unsigned int i = 0; i < indices.size() && nCut < params.maxCutPerRound && nCut < cached_.nBasics_; i++) {
...
public:
  ...
  {
    return (t1.third < t2.third) || ((t1.third == t2.third) && (t1.second < t2.second));
  }
};
...
struct StableExternalComp {
  ...
  {
    bool result = (vec_1_[i] < vec_1_[j]) || (((vec_1_[i] == vec_1_[j])) && (vec_2_[i] < vec_2_[j]));
    return result;
  }
};
void CglLandP::getSortedFractionalIndices(std::vector< int > &frac_indices,
  ...
  const CglLandP::Parameters &params) const
{
  ...
  for (int i = 0; i < data.nBasics_; i++) {
    ...
    if (iCol >= data.nNonBasics_ || !data.integers_[iCol] || INT_INFEAS(data.colsol_[iCol]) <= params.away)
      continue;
