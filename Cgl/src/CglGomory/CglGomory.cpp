  ...
  const CglTreeInfo info)
{
  ...
  if (clpSolver) {
    ...
    if (whenToDo == 2 || (numberRows > numberOriginalRows && whenToDo == 1 && (info.options & 512) == 0) || ((info.options & 1024) != 0 && (info.options & 512) == 0 && numberTimesStalled_ < 3)) {
      // bounds
  ...
  const CglTreeInfo info)
{
...
#ifdef CLP_OSL
  double relaxation = !alternateFactorization_ ? factorization.conditionNumber() : factorization2->conditionNumber();
#else
  ...
  for (iColumn = 0; iColumn < numberColumns; iColumn++) {
    if (columnIsBasic[iColumn] < 0 && colUpper[iColumn] - colsol[iColumn] <= tolerance) {
      swap[iColumn] = true;
    } else {
      swap[iColumn] = false;
    }
    ...
    if (rowIsBasic[iRow] < 0 && rowUpper[iRow] > rowLower[iRow] + 1.0e-7) {
      ...
      if (rowActivity[iRow] >= rowUpper[iRow] - 1.0e-7) {
        ...
        assert(CoinMin(rowUpper[iRow] - rowActivity[iRow],
                 rowActivity[iRow] - rowUpper[iRow])
          < 1.0e-5);
        //abort();
        continue;
#else
...
#if CGL_DEBUG > 1
              if (iColumn == 52)
                printf("%d value %g reduced %g int %d rhs %g swap %d\n",
...
#define LARGE_BOUND 1.0e20
        ...
        if (difference2 < useTolerance && doSorted && number < saveLimit)
          ...
          accurate2 = true;
#endif
        if (sum > rhs + tolerance2 * away && (accurate || accurate2)) {
          //#ifdef CGL_DEBUG
#ifdef CGL_DEBUG
#if CGL_DEBUG <= 1
          if (number <= 10) {
...
#define TRY7 1
          ...
          if ((!numberNonInteger || number < 6) && number && USE_CGL_RATIONAL >= 0) {
            ...
            for (j = 0; j < number + 1; j++) {
              ...
              lcm /= thisGcd;
              if (static_cast< double >(lcm) * cleaned[j].denominator < 1.0e18) {
...
#define CGL_GOMORY_TINY_ELEMENT 1.0e-12
#endif
          {
            ...
            if (largest > 1.0e10 * smallest || (number > 20 && smallest < number * 1.0e-6) || numberNonInteger < -10 || !number) {
...
#define PRINT_NUMBER 0
              ...
              for (int i = 0; i < number; i++) {
                if (fabs(packed[i] - floor(packed[i] + 0.5)) > 1.0e-14 || fabs(packed[i]) > 1.0e4)
                  dontScale = false;
              }
              if (!dontScale) {
...
int CglGomory::maximumLengthOfCutInTree() const
{
  if (limit_)
    return limit_;
  else
...
void CglGomory::refreshSolver(OsiSolverInterface *solver)
{
