...
//-------------------------------------------------------------
void CglSimpleRounding::generateCuts(const OsiSolverInterface &si, OsiCuts &cs,
  const CglTreeInfo /*info*/)
{
  ...
  const CoinPackedMatrix *rowCopy = si.getMatrixByRow(); // row copy: matrix stored in row order
  ...
  for (rowIndex = 0; rowIndex < nRows; rowIndex++) {
    ...
    if (!deriveAnIntegerRow(si,
      ...
      for (k = 0; k < irow.getNumElements(); k++)
        negative[irow.getIndices()[k]] = false;
...
//-------------------------------------------------------------------
bool CglSimpleRounding::deriveAnIntegerRow(
  ...
  bool *negative) const
{
  ...
  for (i = 0; i < sizeOfRow; i++) {
    ...
    // else: the variable is integer
      ...
      if (colupper[matrixRow.getIndices()[i]] - collower[matrixRow.getIndices()[i]] < epsilon_) {
        b = b - (sign * matrixRow.getElements()[i] * colupper[matrixRow.getIndices()[i]]);
      }
...
//-------------------------------------------------------------------
//
int CglSimpleRounding::power10ToMakeDoubleAnInt(
