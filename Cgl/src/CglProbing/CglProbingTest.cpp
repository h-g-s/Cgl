...

  {
      ...
      for (i = 0; i < nColCuts; i++) {
        ...
        for (j = 0; j < n; j++) {
          ...
          if (elements[j] > lo[icol])
            std::cout << "Can increase lb on " << icol << " from " << lo[icol] << " to " << elements[j] << std::endl;
        }
        cpv = ccut.ubs();
        ...
        for (j = 0; j < n; j++) {
          ...
          if (elements[j] < up[icol])
            std::cout << "Can decrease ub on " << icol << " from " << up[icol] << " to " << elements[j] << std::endl;
        }
      }
    ...
    for (i = 0; i < nRowCuts; i++) {
      ...
      if (sum2 > ub + 1.0e-7 || sum2 < lb - 1.0e-7) {
        ...
        for (k = 0; k < n; k++) {
          int column = indices[k];
          std::cout << "(col=" << column << ",el=" << elements[k] << ",sol=" << colsol[column] << ") ";
        }
        std::cout << std::endl;
      }
    }
