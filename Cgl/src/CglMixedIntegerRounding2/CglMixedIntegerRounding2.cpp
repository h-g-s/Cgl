...
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::generateCuts(const OsiSolverInterface &si,
  OsiCuts &cs,
...
// Generate MIR cuts
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::generateMirCuts(
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::copyRowSelected(
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding2::selectRowToAggregate(
  //const OsiSolverInterface& si,
  ...
  int &colSelected) const
{
  ...
  for (int j = 0; j < numColsAggregated; ++j) {
    ...
    if ((integerType_[indCol]) || (fabs(coefCol) < EPSILON_))
      continue;
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::aggregateRow(
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding2::boundSubstitution(
  const OsiSolverInterface &si,
  const CoinIndexedVector &rowAggregated,
  const double *xlp,
  ...
  CoinIndexedVector &contVariablesInS) const
{
  ...
  for (j = 0; j < numInt; ++j) {
    ...
    if (fabs(knapsackElements[indCol]) < EPSILON_)
      continue;
    // if the lower bound is not zero, then we stop
    if (fabs(colLowerBound[indCol]) > EPSILON_)
      return generated;
  }
...
//-------------------------------------------------------------------
bool CglMixedIntegerRounding2::cMirSeparation(
  const OsiSolverInterface &si,
  const CoinPackedMatrix &matrixByRow,
  const CoinIndexedVector &rowAggregated,
...
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::cMirInequality(
  const int numInt,
  const double delta,
...
// Printing statistics
//-------------------------------------------------------------------
void CglMixedIntegerRounding2::printStats(
  std::ofstream &fout,
  const bool hasCut,
  const OsiSolverInterface &si,
  const CoinIndexedVector &rowAggregated,
...
void CglMixedIntegerRounding2::refreshSolver(OsiSolverInterface *solver)
{
