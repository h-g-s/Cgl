...
bool CglLandPSimplex::checkBasis()
{
...
void CglLandPSimplex::computeWeights(CglLandP::LHSnorm norm, CglLandP::Normalization type,
  CglLandP::RhsWeightType rhs)
{
  ...
  } else if (norm == CglLandP::L1 || norm == CglLandP::Average) {
    for (int i = 0; i < ncols_orig_; i++) {
      CoinBigIndex begin = start[i];
      CoinBigIndex end = begin + length[i];
      for (CoinBigIndex k = begin; k < end; k++) {
...
void CglLandPSimplex::cacheUpdate(const CglLandP::CachedData &cached, bool reducedSpace)
{
...
int CglLandPSimplex::generateExtraCuts(const CglLandP::CachedData &cached,
  const CglLandP::Parameters &params)
{
  int ret_val = 0;
...
int CglLandPSimplex::generateExtraCut(int i, const CglLandP::CachedData &cached,
  const CglLandP::Parameters &params)
{
  const int &iCol = basics_[i];
  if (!isInteger(iCol) || int_val(colsol_[iCol], params.away) || !int_val(getColsolToCut(iCol), params.away) || colsol_[iCol] < getLoBound(iCol) || colsol_[iCol] > getUpBound(iCol) || (cuts_.rowCut(iCol) != NULL)) {
    return false;
  }
...
void CglLandPSimplex::genThisBasisMigs(const CglLandP::CachedData &cached,
  const CglLandP::Parameters &params)
{
  for (int i = 0; i < cached.nBasics_; i++) {
    const int iCol = basics_[i];
    if (iCol >= ncols_ || !cached.integers_[iCol] || int_val(colsol_[iCol], params.away))
      continue;
    OsiRowCut *cut = new OsiRowCut;
    generateMig(i, *cut, params);
    int code = validator_(*cut, cached.colsol_, *si_, params, &lo_bounds_[0], &up_bounds_[0]);
...
bool CglLandPSimplex::generateMig(int row, OsiRowCut &cut,
  const CglLandP::Parameters &params)
{
  row_k_.num = row;
...
bool CglLandPSimplex::optimize(int row, OsiRowCut &cut, const CglLandP::CachedData &cached, const CglLandP::Parameters &params)
{
  bool optimal = false;
  int nRowFailed = 0;
  ...
  while (!optimal && numPivots < params.pivotLimit) {
...
#endif
      if (leaving >= 0) {
        if (params.pivotSelection == CglLandP::mostNegativeRc || (params.pivotSelection == CglLandP::initialReducedCosts && numPivots == 0)) {
          ...
          while (incoming < 0 && !optimal && nRowFailed < maxTryRow) // if no improving was found rescan the tables of reduced cost to find a good one
          {
            if (incoming == -1 || params.countMistakenRc)
              nRowFailed++;
            rowFlags_[leaving] = false;
            hasFlagedRow = true;
    ...
    if (leaving >= 0) {
      if (incoming >= 0 && !optimal) {
        ...
        bool recompute_source_row = (numPivots && (numPivots % 10 == 0 || fabs(gamma) < 1e-05));
#endif
        ...
        if (params.generateExtraCuts == CglLandP::WhenEnteringBasis && basics_[leaving] < ncols_ && cuts_.numberCuts() < params.extraCutsLimit)
        ...
        if (pivoted) {
          ...
          handler_->message(PivotLog, messages_) << numPivots << sigma_ << nonBasics_[incoming] << basics_[leaving] << direction << gamma << inDegenerateSequence_ << CoinMessageEol << CoinMessageEol;
        } else //pivot failed
        {
          numFailedPivots++;
...
bool CglLandPSimplex::changeBasis(int incoming, int leaving, int leavingStatus,
#ifndef OLD_COMPUTATION
  ...
  bool modularize)
{
  ...
  else //Update row k by hand
  {
    ...
    if (1 || fabs(gamma) > 1e-9) {
      ...
      for (int i = 0; i < nnz; i++) {
        if (row_k_.getNumElements() > row_k_.capacity() - 2)
          row_k_.scan();
...
int CglLandPSimplex::findCutImprovingPivotRow(int &direction, int &gammaSign, double tolerance)
{
  bool bestRed = 0;
...
int CglLandPSimplex::fastFindCutImprovingPivotRow(int &direction, int &gammaSign,
  double tolerance, bool flagPositiveRows)
{
  ...
  for (unsigned int i = 0; i < M1_.size(); i++) {
    ...
    if (ii < ncols_) {
      ...
      bool swap = false;
      if (basis_->getStructStatus(ii) == CoinWarmStartBasis::atUpperBound)
        swap = true;
      for (CoinBigIndex k = begin; k < end; k++) {
        if (swap) {
      ...
      bool swap = false;
      if (basis_->getArtifStatus(ii - ncols_orig_) == CoinWarmStartBasis::atUpperBound)
        swap = true;
      if (swap) {
  ...
  for (unsigned int i = 0; i < M2_.size(); i++) {
    ...
    if (ii < ncols_) {
      ...
      bool swap = false;
      if (basis_->getArtifStatus(M2_[i] - ncols_orig_) == CoinWarmStartBasis::atUpperBound)
        swap = true;
      if (swap) {
  ...
  for (unsigned int i = 0; i < M3_.size(); i++) {
    ...
    bool swap = false;
    if (ii < ncols_orig_ && basis_->getStructStatus(ii) == CoinWarmStartBasis::atUpperBound)
      swap = true;
    if (ii >= ncols_orig_ && basis_->getArtifStatus(ii - ncols_orig_) == CoinWarmStartBasis::atUpperBound)
      swap = true;
  ...
  for (int i = 0; i < nrows_; i++) {
    ...
    if (loBound > -1e50) {
      redCost = -normedCoef(sigma, basics_[i]) + (tau1)
        + (1 - fzero) * (colsol_[basics_[i]] - loBound);

      if (redCost < -tolerance) {
        ul_i[i] = redCost;
        hasNegativeRc = true;
      } else {
        if (fabs(redCost) < tolerance)
          nZeroRc++;
        else
          nPositiveRc++;
        ul_i[i] = 10.;
    ...
    if (getUpBound(basics_[i]) < 1e50) {
      redCost = -normedCoef(sigma, basics_[i]) - (tau3)
        + (1 - fzero) * (-colsol_[basics_[i]] + upBound);
  ...
  if (bestRow != -1) {
    ...
    row_i_.num = bestRow;
    pullTableauRow(row_i_);
    handler_->message(FoundImprovingRow, messages_) << bestRow << basics_[bestRow] << direction << gammaSign << bestReducedCost
                                                    << CoinMessageEol;
  }
  assert(bestRow < 0 || direction != 0);
  return bestRow;
...
/** Find a row which can be used to perform an improving pivot tables are already filled.
 \return index of the cut or -1 if none exists. */
int CglLandPSimplex::rescanReducedCosts(int &direction, int &gammaSign, double tolerance)
{
...
void CglLandPSimplex::compute_p_q_r_s(double gamma, int gammaSign, double &p, double &q, double &r, double &s)
{
  for (int i = 0; i < ncols_; i++) {
    const int &ii = nonBasics_[i]; //True index
    if (colCandidateToLeave_[i] == false)
      continue;
...
int CglLandPSimplex::fastFindBestPivotColumn(int direction, int gammaSign,
...
int CglLandPSimplex::findBestPivotColumn(int direction,
  double pivotTol, bool reducedSpace, bool allowDegenerate, bool modularize)
{
  ...
  for (; j < ncols_orig_; j++) {
    if (reducedSpace && !colCandidateToLeave_[j])
      continue;
  ...
  const CglLandP::Parameters &params)
{
  ...
  for (int l = 0; l < k && l < 10; l++, notImproved++) {
    if (!rowFlags_[rc[l].row])
      continue; //this row has been marked to be skipped
    ...
    double sigma;
    int incoming = fastFindBestPivotColumn(rc[l].direction, rc[l].gammaSign,
      params.pivotTol, params.away,
      (params.sepSpace == CglLandP::Fractional),
      0,
    ...
    if (rc[l].direction == 2 || rc[l].direction == -2) {
      ...
      //checks
      incoming = fastFindBestPivotColumn(rc[l].direction, rc[l].gammaSign2,
        params.pivotTol, params.away,
        (params.sepSpace == CglLandP::Fractional),
        0,
        sigma, params.modularize);
...
CglLandPSimplex::computeCglpObjective(double gamma, bool strengthen)
{
  ...
  numerator += (coeff > 0 ? coeff * (1 - rhs) : -coeff * rhs) * getColsolToCut(basics_[row_i_.num]);
  for (int j = 0; j < ncols_; j++) {
    ...
    numerator += (coeff > 0 ? coeff * (1 - rhs) : -coeff * rhs) * getColsolToCut(nonBasics_[j]);
  }
  return numerator * rhs_weight_ / denominator;
}
...
CglLandPSimplex::computeCglpObjective(double gamma, bool strengthen, TabRow &newRow)
{
  ...
  int k = 0;
  {
    ...
    numerator += (val > 0 ? val * (1 - newRow.rhs) : -val * newRow.rhs) * getColsolToCut(basics_[row_i_.num]);
  }
  for (int j = 0; j < ncols_; j++) {
    ...
    numerator += (val > 0 ? val * (1 - newRow.rhs) : -val * newRow.rhs) * getColsolToCut(nonBasics_[j]);
  }
...
CglLandPSimplex::computeCglpRedCost(int direction, int gammaSign, double tau)
{
  ...
  value = -sigma_ + Tau
    + (1 - getColsolToCut(basics_[row_k_.num])) * sign * (row_i_.rhs - toBound)
...
void CglLandPSimplex::updateM1_M2_M3(TabRow &row, double tolerance, bool perturb)
{
  M1_.clear();
  M2_.clear();
  M3_.clear();
...
void CglLandPSimplex::createIntersectionCut(TabRow &row, OsiRowCut &cut) const
{
    ...
    if (fabs(row[nonBasics_[j]]) > 1e-10) {
      ...
      if (nonBasics_[j] < ncols_) {
        ...
        if (rowLower[iRow] > -infty) {
          ...
          assert(basis_->getArtifStatus(iRow) == CoinWarmStartBasis::atUpperBound || (fabs(rowLower[iRow] - rowUpper[iRow]) < 1e-08));
        } else {
...
void CglLandPSimplex::createMIG(TabRow &row, OsiRowCut &cut) const
{
  const double *colLower = si_->getColLower();
  const double *rowLower = si_->getRowLower();
  const double *colUpper = si_->getColUpper();
      ...
      if (fabs(row[nonBasics_[j]]) > 0.) {
        {
          if (nonBasics_[j] < ncols_orig_) {
            ...
            if (rowUpper[iRow] < infty) {
              ...
              value = -value;
              cutRhs -= value * rowLower[iRow];
              assert(basis_->getArtifStatus(iRow) == CoinWarmStartBasis::atUpperBound || (rowUpper[iRow] < infty));
            }
            vec[original_index_[nonBasics_[j]]] = value;
            assert(fabs(cutRhs) < 1e100);
          }
...
void CglLandPSimplex::eliminate_slacks(double *vec) const
{
  const CoinPackedMatrix *mat = si_->getMatrixByCol();
  const CoinBigIndex *starts = mat->getVectorStarts();
  const int *lengths = mat->getVectorLengths();
...
void CglLandPSimplex::scaleCut(OsiRowCut &cut, double factor) const
{
...
#ifdef APPEND_ROW
void CglLandPSimplex::append_row(int row_num, bool modularize)
{
...
void CglLandPSimplex::check_mod_row(TabRow &row)
{
...
void CglLandPSimplex::update_row(TabRow &row)
{
  int r_idx = nrows_ - 1;
  int c_idx = ncols_ - 1;
...
void CglLandPSimplex::pullTableauRow(TabRow &row) const
{
  const double *rowLower = si_->getRowLower();
  const double *rowUpper = si_->getRowUpper();
...
void CglLandPSimplex::adjustTableauRow(int var, TabRow &row, int direction)
{
  double bound = 0;
  assert(direction != 0);
  if (direction > 0) {
...
void CglLandPSimplex::resetOriginalTableauRow(int var, TabRow &row, int direction)
{
  if (direction > 0) {
...
void CglLandPSimplex::removeRows(int nDelete, const int *rowsIdx)
{
...
void CglLandPSimplex::printEverything()
{
