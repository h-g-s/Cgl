  ...
  int whichTab) const
{
  ...
  for (int i = 0; i < mTab; ++i) {
    ...
    if ((i != rowIndex) && (norm[i] > param.getNormIsZero())) {
      ...
      bool match = false;
      for (int j = 0; j < nTab; ++j) {
        if ((fabs(workNonBasicTab[rowIndex][j]) > param.getEPS_COEFF()) && (fabs(workNonBasicTab[i][j]) > param.getEPS_COEFF())) {
          match = true;
          break;
        }
      }
      ...
      if (whichTab == 0 || whichTab == 2) {
        for (int j = 0; j < card_intNonBasicVar; ++j) {
          if ((fabs(intNonBasicTab[rowIndex][j]) <= param.getEPS_COEFF()) && (fabs(intNonBasicTab[i][j]) > param.getEPS_COEFF())) {
  ...
  int whichTab) const
{
  ...
  for (int i = 0; i < mTab; ++i) {
    if ((i != rowIndex) && (norm[i] > param.getNormIsZero())) {
      ...
      if (whichTab == 0 || whichTab == 2) {
#if RS_FAST_INT == 0
        denom += initnorm * rs_dotProd(intNonBasicTab[i], intNonBasicTab[i], card_intNonBasicVar);
#elif RS_FAST == 1
        denom += initnorm * rs_dotProd(intNonBasicTab[i], intNonBasicTab[i], card_intNonBasicVar);
        double value = initnorm * rs_sparseDotProd(intNonBasicTab[i], intNonBasicTab[i], pi_mat[i] + mTab, pi_mat[i] + mTab);
        assert(value == denom);
#else
        denom = initnorm * rs_sparseDotProd(intNonBasicTab[i], intNonBasicTab[i], pi_mat[i] + mTab, pi_mat[i] + mTab);
#endif
      }
      if (whichTab == 1 || whichTab == 2) {
...
void CglRedSplit2::fill_workNonBasicTab(CglRedSplit2Param::ColumnSelectionStrategy strategy, const int *ignore_list)
{
  ...
  if (strategy == CglRedSplit2Param::CS_ALLCONT) {
    ...
    for (i = 0; i < card_contNonBasicVar; ++i) {
      if (ignore_list != NULL) {
        iter = 0;
        while (ignore_list[iter] >= 0 && ignore_list[iter] != contNonBasicVar[i]) {
          iter++;
        }
  ...
  for (k = 0; k < mTab && k < maxIterations; ++k) {
    ...
    if (norm[k] > param.getNormIsZero()) {
      ...
      if (sumnorm == 1) {
        ...
        continue;
      } else if (!resolveWithNormalization && sumnorm > param.getMaxSumMultipliers()) {
...
int CglRedSplit2::check_dynamism(double *row)
{
  ...
  if ((max_val < param.getMAXDYN() * min_val) && (max_val >= min_val)) {
    return 1;
  }
  ...
  int *card_row, double &rhs)
{
  int i;
  double value;
  int max_support = param.getMAX_SUPP_ABS() + (static_cast< int >(ncol * param.getMAX_SUPP_REL()));
...
int CglRedSplit2::generateCuts(OsiCuts *cs, int maxNumCuts, int *lambda)
{
  ...
  for (i = 0; i < nrow; i++) {
    ...
    if (basis_index[ind_row] >= ncol || cv_intBasicVar[basis_index[ind_row]] != 1) {
  ...
  rs_allocmatINT(&pi_mat, mTab, mTab + card_intNonBasicVar + 2 * card_contNonBasicVar + 3);
  for (int i = 0; i < mTab; i++) {
    int *info = pi_mat[i];
  ...
  std::vector< CglRedSplit2Param::ColumnSelectionStrategy > listColSel = param.getColumnSelectionStrategy();
  std::vector< CglRedSplit2Param::RowSelectionStrategy > listRowSel = param.getRowSelectionStrategy();
...
void CglRedSplit2::compute_is_integer()
{
  ...
  for (i = 0; i < ncol; i++) {
    if (solver->isInteger(i)) {
      ...
      if ((colUpper[i] - colLower[i] < param.getEPS()) && (rs_above_integer(colUpper[i]) < param.getEPS())) {
...
bool CglRedSplit2::needsOptimalBasis() const
{
  return true;
}
