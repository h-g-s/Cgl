...
void CglFlowCover::flowPreprocess(const OsiSolverInterface &si)
{
  ...
  for (iRow = 0; iRow < numRows; ++iRow) {
    ...
    if ((rowType2 == CGLFLOW_ROW_VARUB) || (rowType2 == CGLFLOW_ROW_VARLB) || (rowType2 == CGLFLOW_ROW_VAREQ)) {
  ...
  const CglTreeInfo info)
{
  ...
  for (iRow = 0; iRow < numRows_; ++iRow) {
    ...
    if ((rType != CGLFLOW_ROW_MIXUB) && (rType != CGLFLOW_ROW_MIXEQ) && (rType != CGLFLOW_ROW_NOBINUB) && (rType != CGLFLOW_ROW_NOBINEQ) && (rType != CGLFLOW_ROW_SUMVARUB) && (rType != CGLFLOW_ROW_SUMVAREQ))
      continue;
  ...
  double &violation)
{
  ...
  if (sense == 'G')
    flipRow(rowLen, coef, rhs); // flips everything,
  // but the sense
  ...
  if (CGLFLOW_DEBUG) {
    ...
    std::cout << "coef [var_index]"
              << " -- "
              << "xlp[var_index]" << '\t' << "vub_coef[vub_index] vub_lp_value OR var_index_col_ub" << std::endl;
  ...
  for (i = 0; i < rowLen; ++i) {
    ...
    VLB = getVlbs(ind[i]);
    LB = (VLB.getVar() != UNDEFINED_) ? VLB.getVal() : si.getColLower()[ind[i]];

    VUB = getVubs(ind[i]);
    UB = (VUB.getVar() != UNDEFINED_) ? VUB.getVal() : si.getColUpper()[ind[i]];
  ...
  while (tempSum < knapRHS + EPSILON_) {
    ...
    for (i = 0; i < rowLen; i++) { // Search the col with  minimum ratio
      if (candidate[i] == CGLFLOW_COL_PRIME && label[i] == 0 && ratio[i] < tempMin) {
        tempMin = ratio[i];
        xID = i;
      }
    }
  ...
  for (int i = 0; i < rowLen; ++i) {
    if ((label[i] == CGLFLOW_COL_INCUT && sign[i] > 0) || label[i] == CGLFLOW_COL_INLMIN) { //  C+ || L-
      // possible
      M[nLook] = -up[i];
      order[nLook++] = i;
    }
  ...
  if (violation > TOLERANCE_) {
    ...
    for (i = 0; i < rowLen; ++i) {
      ...
      if ((sign[i] == CGLFLOW_COL_BINPOS) || (sign[i] == CGLFLOW_COL_BINNEG)) {
...
//-------------------------------------------------------------------
void CglFlowCover::flipRow(int rowLen, double *coef, double &rhs) const
{
  for (int i = 0; i < rowLen; ++i)
    coef[i] = -coef[i];
  rhs = -rhs;
...
//-------------------------------------------------------------------
void CglFlowCover::flipRow(int rowLen, double *coef, char &sen,
  double &rhs) const
{
  for (int i = 0; i < rowLen; ++i)
    coef[i] = -coef[i];
...
void CglFlowCover::liftMinus(double &movement, /* Output */
  int t,
  int r,
  double z,
  double dPrimePrime,
