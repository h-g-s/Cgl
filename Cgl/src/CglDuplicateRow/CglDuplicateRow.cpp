  ...
  const CglTreeInfo info)
{
          ...
          for (int j = last; j < i; j++) {
            ...
            for (int k = j + 1; k < i; k++) {
              ...
              for (kk = columnStart[kColumn]; kk < columnStart[kColumn] + columnLength[kColumn]; kk++) {
                int iRow = row[kk];
                if (random[iRow]) {
                  if (iRow != rowEqualJ[nEqualK] || element[kk] != elementEqualJ[nEqualK]) {
                    dominate = -2;
                    break;
                  } else {
                    nEqualK++;
  ...
  for (i = 0; i < numberRows; i++) {
    if (duplicate_[i] == -5) {
      if ((rowLower[i] <= 0.0 || rowLower[i] == rowUpper[i]) && rowUpper[i] == floor(rowUpper[i])) {
    ...
    for (i = 0; i < numberRows; i++) {
      if (rowLength[i] == 2 && (duplicate_[i] < 0 && duplicate_[i] != -2)) {
        ...
        if (possible) {
          ...
          if (element0 == 1.0 && element1 == 1.0 && rowLower[i] == 1.0 && rowUpper[i] > 1.0e30) {
            if (logLevel_) {
...
#define CGL_INVESTIGATE
		...
		*/
                ...
                if (same || redundant || !feasible || fixed[0] || fixed[1] || tighter0 || tighter1) {
                  ...
                  if (!feasible) {
                    ...
                    assert(same);
                    if (fabs(effectiveRhs[i] - effectiveRhs[k]) < 1.0e-7 && element0[1] == element1[1] && element0[0] == element1[0]) {
#ifdef CGL_INVESTIGATE
		...
		*/
                ...
                if (same || redundant || !feasible || fixed[0] || fixed[1] || fixed[2] || tighter0 || tighter1) {
#ifdef CGL_INVESTIGATE
                  /* start debug print */
                  {
                    printf("Base %d (orig %d) ", i, originalRows[i]);
                  ...
                  if (!feasible) {
                    ...
                    assert(same);
                    if (fabs(effectiveRhs[i] - effectiveRhs[k]) < 1.0e-7 && element0[2] == element1[2] && element0[1] == element1[1] && element0[0] == element1[0]) {
#ifdef CGL_INVESTIGATE
	      ...
	      */
              ...
              for (int jj = 0; jj < (1 << nInt); jj++) {
                ...
                if (fabs(upC0[jj] - upC1[jj]) > 1.0e-8)
                  same = false;
                if (loC0[jj] < loC1[jj] - 1.0e-12 || upC0[jj] > upC1[jj] + 1.0e-12) {
                  tighter0 = false;
                  redundant = false;
                }
                if (loC1[jj] < loC0[jj] - 1.0e-12 || upC1[jj] > upC0[jj] + 1.0e-12) {
                  tighter1 = false;
                  redundant = false;
                }
              ...
              if (same || redundant || !feasible || fixed[0] || fixed[1] || tighter0 || tighter1) {
                ...
                if (!feasible) {
                  ...
                  assert(same);
                  if (fabs(effectiveRhs[i] - effectiveRhs[k]) < 1.0e-7 && el0[2] == el1[2] && el0[0] == el1[0]) {
                  ...
                  if (duplicate_[k] != -2) {
                    if (nInt == 1) {
                      // one may be stronger
                      if (el0[2] > 0.0 && el1[2] > 0.0 && el0[0] > 0.0 && el1[0] > 0.0 && (rowFlag[k] & 1) == 0 && (rowFlag[i] & 1) == 0) {
                        ...
                        if ((rowFlag[k] & 1) == 0 && (rowFlag[i] & 1) == 0) {
                          ...
                          for (int i0 = 0; i <= 10; i++) {
                            ...
                            if (fabs(lo0 - lo1) > 1.0e-8 || fabs(up0 - up1) > 1.0e-8 * (1.0 + fabs(up1))) {
                              if (lo0 > lo1 + 1.0e-8) {
                                if (up0 < up1 - 1.0e-8) {
                                  // 0 tighter
                                  if (which == 1) {
...
void CglDuplicateRow::refreshSolver(OsiSolverInterface *solver)
{
  delete[] rhs_;
  delete[] duplicate_;
  delete[] lower_;
