  ...
  const CglTreeInfo info)
{
  ...
  if (!clpSolver) {
    ...
    if (whenToDo == 2 || (numberRows > numberOriginalRows && whenToDo == 1 && (info.options & 512) == 0) || ((info.options & 1024) != 0 && (info.options & 512) == 0)) {
      // bounds
	...
	if (intVar[i]==1) {
      ...
      CoinWarmStartBasis *warm = dynamic_cast< CoinWarmStartBasis * >(warmstart);
  ...
  if (debugger && !debugger->onOptimalPath(si))
    ...
    if (talk)
      printf("2mir_test: debug success\n");
  }
#endif
         ...
         and variables participating in the constraint are also integer.    */
      {
        ...
        if (DGG_isConstraintBoundedAbove(data, j)) {
          if (frac_part(rowUpper[i]) > DGG_INTEGRALITY_THRESH)
            goto DONE_ROW;
        } else if (frac_part(rowLower[i]) > DGG_INTEGRALITY_THRESH)
          goto DONE_ROW;
...
int DGG_getTableauConstraint(int index, const void *osi_ptr, DGG_data_t *data,
...
int DGG_getFormulaConstraint(int da_row,
  const void *osi_ptr,
  DGG_data_t *data,
  DGG_constraint_t *form_row)
{
  /* ensure that the da_row corresponds to a row */
  if (data->nrow <= da_row || 0 > da_row)
    DGG_THROW(1, "row out of range...");
  ...
  int i;
  for (i = 0; i < nz; i++)
    form_row->coeff[i] = rowMat[rowBeg[da_row] + i];
  for (i = 0; i < nz; i++)
    form_row->index[i] = rowInd[rowBeg[da_row] + i];
...

{
  ...
  for (i = 0; i < cut->nz; i++) {
    ...
    if (DGG_isInteger(data, cut->index[i])) { // look at integral vars.
      ...
      cut->coeff[i] = 0.0;
    else if (cut->coeff[i] < DGG_NICEFY_MIN_FIX) { // coefficient = epsylon
...
/******************* CUT GENERATION *******************************************/
int DGG_generateTabRowCuts(DGG_list_t *cut_list,
  DGG_data_t *data,
...

{
  ...
  if (talk)
    printf("2mir_test: generating tab row cuts\n");
  ...
  int i;
  for (i = 0; i < data->ncol; i++) {
    if (DGG_isBasic(data, i))
      colIsBasic[i] = 1;
    else
      colIsBasic[i] = -1;
  }
  for (i = 0; i < data->nrow; i++) {
    if (DGG_isBasic(data, i + data->ncol))
      rowIsBasic[i] = 1;
    else
      rowIsBasic[i] = -1;
  }
  ...
  for (k = 0; k < data->ncol; k++) {
    if (!(DGG_isBasic(data, k) && DGG_isInteger(data, k)))
      continue;

    double frac = frac_part(data->x[k]);
    if (frac < data->gomory_threshold || frac > 1 - data->gomory_threshold)
      continue;
  ...
  CoinThreadRandom &generator)
{
  ...
  for (p = 0; p < base->nz; p++) {
    if (isint[p])
      if (generator.randomDouble() < prob_choose) {
        if (xout[p] < 0.01)
          continue;
...
int DGG_generateCutsFromBase(DGG_constraint_t *orig_base,
  DGG_list_t *cut_list,
  DGG_data_t *data,
  const void *solver_ptr)
{
  ...
  int min_t = t_min;
  int min_q = q_min;
  if (orig_base->sense == 'G' && min_t < 1)
    min_t = 1;
  if (orig_base->sense == 'G' && min_q < 1)
    min_q = 1;
  ...
  for (t = min_t; t <= t_max; t++) {
    ...
    if (DGG_isBaseTrivial(data, base))
      goto MIR_DONE;
  ...
  DGG_constraint_t * /*orig_base*/)
{
  ...
  for (i = 0; i < base->nz; i++)
    if (isint[i])
      best_rc = CoinMax(best_rc, fabs(rc[i]));
  double rc_cutoff = best_rc / 10;

  for (i = 0; i < base->nz; i++) {
    if (!isint[i])
      continue;
    if (fabs(rc[i]) <= rc_cutoff)
      continue; //too unimportant
...
bool CglTwomir::needsOptimalBasis() const
{
  return true;
}
...
void CglTwomir::refreshSolver(OsiSolverInterface *solver)
{
  if (originalSolver_) {
    delete originalSolver_;
    originalSolver_ = solver->clone();
...
CglTwomir::generateCpp(FILE *fp)
{
  ...
  if (do_mir_ != other.do_mir_ || do_2mir_ != other.do_2mir_ || do_tab_ != other.do_tab_ || do_form_ != other.do_form_)
