...
void CglRedSplit::update_redTab(int r1, int r2, int step)
{
  ...
  int j;
  for (j = 0; j < nTab; j++) {
    contNonBasicTab[r1][j] = contNonBasicTab[r1][j] - step * contNonBasicTab[r2][j];
  }
} /* update_redTab */
        ...
        for (j = i + 1; j < mTab; j++) {
          if (norm[j] > param.getNormIsZero()) {
            if ((checked[i][j] < changed[i]) || (checked[i][j] < changed[j])) {
              ...
              if ((checked[j][i] < changed[i]) || (checked[j][i] < changed[j])) {
...
double CglRedSplit::row_scale_factor(double *row)
{
  ...
  if (has_lub) {
    if ((max_val > param.getEPS_COEFF_LUB()) && (max_val < param.getMAXDYN_LUB() * min_val) && (max_val >= min_val)) {
      return (retval);
    }
  } else {
    if ((max_val > param.getEPS_COEFF()) && (max_val < param.getMAXDYN() * min_val) && (max_val >= min_val)) {
      return (retval);
    }
  }
...
void CglRedSplit::generateCuts(OsiCuts &cs)
{
  ...
  for (i = 0; i < ncol; i++) {
    ...
    case 1: // basic variable
      ...
      if (is_integer[i] && (dist_int > param.getAway()) && (dist_int < 1 - param.getAway())) {
...
bool CglRedSplit::needsOptimalBasis() const
{
  return true;
}
